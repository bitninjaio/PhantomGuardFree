...
import { Lock, CalendarSearch, Maximize2, Minimize2 } from 'lucide-react';

/**
 * Free tier: origin traffic analytics are always unavailable (Pro feature).
 * Static placeholder only — no chart library or remote loaders.
 */
const OriginTrafficChart = (props) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`transition-all ease-in-out min-w-[300px] phguard-card relative overflow-hidden ${
    ...
    >
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            disabled
            className="flex cursor-not-allowed items-center gap-2 rounded p-1.5 text-gray-400 opacity-50 dark:text-gray-500"
            title={t('dashboard.premiumFeature', { defaultValue: 'Premium feature' })}
          >
      ...
      <section
        aria-label={t('dashboard.originTrafficDistribution')}
      ...
      >
        ...
        <h4 className="mb-2 text-lg font-semibold text-gray-900 dark:text-gray-100">{t('dashboard.originTrafficDistribution')}</h4>
        <p className="phguard-placeholder-desc max-w-sm text-sm text-gray-600 dark:text-gray-300">
          {t('dashboard.originTrafficLocked', {
            defaultValue:
              'Origin traffic distribution is available in PhantomGuard Pro. Upgrade to view traffic analytics over time.',
          })}
        </p>
      </section>
    </div>
